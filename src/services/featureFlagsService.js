...
// ── In-Memory Cached Flags for Fast Evaluation ──────────────────────────────
let _cachedFlagsMap = {};

// ── Helper: Write to Global Audit Log ───────────────────────────────────────
...
// ── Helper: Sync to global_config/featureFlags for backward compatibility ──
...
// ── 1. Real-time Subscription to Feature Flags Collection ────────────────────
export function subscribeFeatureFlags(onSuccess, onError) {
  ...
  return onSnapshot(q, async (snapshot) => {
    ...
    // Fallback to local initial flags if network/permissions block
    onSuccess(INITIAL_FEATURE_FLAGS);
    if (onError) onError(error);
  });
}

// ── 2. Seed Initial Flags to Firestore ───────────────────────────────────────
...
// ── 3. Toggle Feature Flag (Instant Enable/Disable) ─────────────────────────
...
// ── 7. Evaluate if Feature is Enabled ───────────────────────────────────────
